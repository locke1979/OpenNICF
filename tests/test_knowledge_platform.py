...
def test_migrations_are_source_controlled_and_versioned():
    ...
    assert [migration_version(path.name) for path in files] == [1, 2, 3]
    assert [path.name for path in files] == ["0001_initial.sql", "0002_hybrid_retrieval.sql", "0003_audit_engine.sql"]
