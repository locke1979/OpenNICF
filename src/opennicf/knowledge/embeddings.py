...
class EmbeddingSpace:
    ...
    def compatible_with(self, other: "EmbeddingSpace") -> bool:
        return self.embedding_space_id == other.embedding_space_id and self.dimension == other.dimension
...
class EmbeddingResult:
    ...
    def space(self) -> EmbeddingSpace:
        return EmbeddingSpace(
            self.embedding_space_id or _space_id(self.model, self.dimensions, self.model_revision),
            self.provider,
            self.model,
            self.model_revision,
            self.dimensions,
...
class HashingEmbeddingBackend:
    ...
    def __init__(self, model: str | None = None, dimensions: int = CANONICAL_DIMENSION, device: str = "cpu"):
        model = model or f"opennicf-hash-{dimensions}"
        self._info = EmbeddingModelInfo(
            model=model, dimensions=dimensions, device=device, backend="hashing", fallback=True,
    ...
    def embed(self, texts: Sequence[str], *, purpose: str = "retrieval_document") -> EmbeddingResult:
        return EmbeddingResult(
            model=self._info.model, dimensions=self._info.dimensions, device=self._info.device,
            vectors=tuple(self._vector_for(text) for text in texts), fallback=True,
            metadata={"fallback_reason": "deterministic_cpu_backend", "purpose": purpose},
...
class QwenEmbeddingBackend:
    ...
    def __init__(self, *, dimension: int = CANONICAL_DIMENSION, model: str = "Qwen/Qwen3-Embedding-0.6B",
                 model_revision: str = "v1", max_input_length: int = 512, batch_size: int = 2,
        ...
        self._info = EmbeddingModelInfo(
            model=model, dimensions=dimension, device=self._detect_device(), backend="qwen3",
            embedding_space_id=_space_id(model, dimension, model_revision), model_revision=model_revision,
        )
    ...
    def _load(self) -> Any:
        ...
        try:
            ...
            model = AutoModel.from_pretrained(self.model, torch_dtype=dtype).to(device).eval()
    ...
    def embed(self, texts: Sequence[str], *, purpose: str = "retrieval_document") -> EmbeddingResult:
        ...
        runtime = self._load()
        if callable(runtime):
            raw = runtime(prepared, purpose=purpose, dimension=self.dimension, device=self._info.device)
            vectors = tuple(_normalize(vector) for vector in raw)
        else:
            tokenizer, model, torch, device = runtime
            with torch.inference_mode():
                batch = tokenizer(prepared, padding=True, truncation=True, max_length=self.max_input_length, return_tensors="pt")
                batch = {key: value.to(device) for key, value in batch.items()}
                output = model(**batch).last_hidden_state
                mask = batch["attention_mask"].unsqueeze(-1)
                pooled = (output * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1)
                vectors = tuple(_normalize(row[: self.dimension].float().cpu().tolist()) for row in pooled)
        if any(len(vector) != self.dimension for vector in vectors):
            raise EmbeddingError("Qwen backend returned the wrong dimensionality")
        return EmbeddingResult(
            model=self.model, dimensions=self.dimension, device=self._info.device, vectors=vectors,
            embedding_space_id=self._info.embedding_space_id, provider="LOCAL", model_revision=self._info.model_revision,
            normalized=True, purpose=purpose, metadata={"instruction_applied": purpose == "retrieval_query", "max_input_length": self.max_input_length},
        )
...
class GeminiEmbeddingBackend:
    ...
    def __init__(self, model: str, *, dimension: int = CANONICAL_DIMENSION, model_revision: str = "v1",
                 api_key: str | None = None, transport: Callable[[str, dict[str, Any]], dict[str, Any]] | None = None):
        if model not in {"gemini-embedding-001", "gemini-embedding-2"}:
            raise ValueError("unsupported Gemini embedding model")
        self.model, self.dimension, self._api_key, self._transport = model, dimension, api_key, transport
        self._info = EmbeddingModelInfo(
            model=model, dimensions=dimension, device="remote", backend="google-gemini",
            embedding_space_id=_space_id(model, dimension, model_revision), provider="GOOGLE", model_revision=model_revision,
        )
    ...
    def _call(self, text: str, purpose: str) -> Sequence[float]:
        ...
        values = response.get("embedding", {}).get("values") or response.get("embeddings", [{}])[0].get("values")
        if not values:
    ...
    def embed(self, texts: Sequence[str], *, purpose: str = "retrieval_document") -> EmbeddingResult:
        ...
        return EmbeddingResult(
            model=self.model, dimensions=self.dimension, device="remote", vectors=vectors,
            embedding_space_id=self._info.embedding_space_id, provider="GOOGLE", model_revision=self._info.model_revision,
            normalized=True, purpose=purpose, metadata={"normalization": "verified_and_enforced", "task_type": purpose},
        )
...
class LocalFirstEmbeddingService:
    ...
    def __init__(self, *, preferred_backend: EmbeddingBackend | None = None,
                 cpu_backend: EmbeddingBackend | None = None, batch_sizer: EmbeddingBatchSizer | None = None,
                 providers: Sequence[EmbeddingBackend] | None = None, active_space_id: str | None = None):
        ...
        self.providers: dict[str, EmbeddingBackend] = {self._provider_space_id(self.cpu_backend): self.cpu_backend}
        for provider in providers or ():
            self.providers[self._provider_space_id(provider)] = provider
        if preferred_backend:
            self.providers[self._provider_space_id(preferred_backend)] = preferred_backend
        self.active_space_id = active_space_id or (self._provider_space_id(preferred_backend) if preferred_backend else self._provider_space_id(self.cpu_backend))
        self.migration_state: dict[str, dict[str, Any]] = {}

    @staticmethod
    def _provider_space_id(provider: EmbeddingBackend) -> str:
        info = provider.info
        return info.embedding_space_id or _space_id(info.model, info.dimensions, info.model_revision)
    ...
    def space(self, space_id: str | None = None) -> EmbeddingSpace:
        ...
        info = provider.info
        return EmbeddingSpace(self._provider_space_id(provider), info.provider, info.model, info.model_revision, info.dimensions, info.normalized)
    ...
    def switch_active(self, space_id: str, *, corpus_ready: bool = False) -> None:
        ...
        if not corpus_ready:
            raise EmbeddingError("active embedding space cannot switch before its corpus/index is validated")
        self.active_space_id = space_id

    def _choose_backend(self, *, space_id: str | None, prefer_gpu: bool) -> EmbeddingBackend:
    ...
    def info(self, *, prefer_gpu: bool = True, available_memory_bytes: int | None = None, available_vram_bytes: int | None = None) -> EmbeddingModelInfo:
        return self._choose_backend(space_id=None, prefer_gpu=prefer_gpu).info

    def embed(self, texts: Sequence[str], *, purpose: str = "retrieval_document", dimension: int | None = None,
              prefer_gpu: bool = True, available_memory_bytes: int | None = None, available_vram_bytes: int | None = None,
              space_id: str | None = None, privacy_policy: str = "cloud_allowed") -> EmbeddingResult:
        if purpose not in RETRIEVAL_PURPOSES:
            raise ValueError(f"purpose must be one of {sorted(RETRIEVAL_PURPOSES)}")
        backend = self._choose_backend(space_id=space_id or self.active_space_id, prefer_gpu=prefer_gpu)
        if privacy_policy == "local_only" and backend.info.provider != "LOCAL":
            raise PrivacyBoundaryError("local_only evidence cannot use a Google embedding space")
        if dimension is not None and dimension != backend.info.dimensions:
            raise EmbeddingError("provider was not configured for the requested dimension")
        limit = min(self.batch_sizer.limit_for(available_memory_bytes=available_memory_bytes,
        ...
        for offset in range(0, len(texts), limit):
            ...
            try:
                ...
                if len(batch) > 1:
                    ...
                    vectors.extend(self.cpu_backend.embed(batch, purpose=purpose).vectors)
                    fallback = True
    ...
    def describe(self) -> dict[str, Any]:
        info = self.info()
        return {"model": info.model, "dimensions": info.dimensions, "device": info.device, "backend": info.backend,
                "fallback": info.fallback, "active_space_id": self.active_space_id,
...
class EmbeddingMigration:
    ...
    def __init__(self, service: LocalFirstEmbeddingService, target_space_id: str, *, batch_size: int = 16):
        if target_space_id not in service.providers:
            raise EmbeddingError(f"target embedding space is not registered: {target_space_id}")
        self.service, self.target_space_id, self.batch_size = service, target_space_id, max(1, batch_size)
        self.state = service.migration_state.setdefault(target_space_id, {"completed": [], "status": "registered"})

    def run(self, chunks: Sequence[Any], persist: Callable[[Any, EmbeddingResult], None], *, privacy_policy: str = "cloud_allowed") -> dict[str, Any]:
        completed = set(self.state.get("completed", []))
        for offset in range(0, len(chunks), self.batch_size):
            batch = [chunk for chunk in chunks[offset:offset + self.batch_size] if getattr(chunk, "chunk_id", chunk) not in completed]
            if not batch:
                continue
            result = self.service.embed([getattr(chunk, "text", str(chunk)) for chunk in batch], purpose="retrieval_document",
                                        space_id=self.target_space_id, privacy_policy=privacy_policy)
            for chunk, vector in zip(batch, result.vectors):
                persist(chunk, EmbeddingResult(model=result.model, dimensions=result.dimensions, device=result.device,
                    vectors=(vector,), fallback=result.fallback, metadata=result.metadata,
                    embedding_space_id=result.embedding_space_id, provider=result.provider,
                    model_revision=result.model_revision, normalized=result.normalized, purpose=result.purpose))
    ...
    def activate(self) -> None:
        if self.state.get("status") != "complete":
            raise EmbeddingError("cannot switch active space before migration is complete")
