...
_DECLARATION_PATTERNS = {
    ...
    "sql": (("table", re.compile(r"\bCREATE\s+(?:TEMP(?:ORARY)?\s+)?TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([\w.]+)", re.I)), ("routine", re.compile(r"\bCREATE\s+(?:OR\s+REPLACE\s+)?(?:FUNCTION|PROCEDURE)\s+([\w.]+)", re.I))),
    "powershell": (("function", re.compile(r"^\s*function\s+([\w.-]+)", re.I)),),
}
_EXTENSIONS = {".py": "python", ".java": "java", ".sql": "sql", ".ps1": "powershell", ".psm1": "powershell"}
_IMPORT_RE = re.compile(r"^\s*(?:import|from)\s+([\w.:-]+)", re.I | re.M)
_CALL_RE = re.compile(r"\b([A-Za-z_]\w*(?:\.[A-Za-z_]\w*)?)\s*\(")
_ENDPOINT_RE = re.compile(r"(?:@(?:app|router)\.(?:get|post|put|delete)|(?:GET|POST|PUT|DELETE)\s+)(?:\(|\s+)?[\"']([^\"']+)", re.I)
_CONFIG_RE = re.compile(r"(?:os\.environ(?:\.get)?|ENV|Get-Item\s+env:|Configuration(?:Manager)?[.]AppSettings)\s*[(\[]?\s*[\"']([A-Z][A-Z0-9_.-]+)", re.I)
...
def build_code_index(content: str, *, source: Any, version: Any, artifact: Any, chunks: Iterable[ChunkRecord]) -> tuple[tuple[CodeSymbolRecord, ...], tuple[CodeRelationshipRecord, ...]]:
    ...
    for symbol in symbols:
        ...
        if re.search(r"\b(?:logger|logging)\.(?:debug|info|warning|error|exception|critical)\b|Write-(?:Verbose|Warning|Error|Information)", body, re.I):
            refs.append(("logging", "logging"))
        if re.search(r"\b(?:try|catch|except|finally)\b", body, re.I):
